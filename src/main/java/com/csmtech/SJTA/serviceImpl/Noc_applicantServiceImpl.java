...
import com.csmtech.SJTA.repository.Noc_applicantRepository;
...
public class Noc_applicantServiceImpl implements Noc_applicantService {
	@Autowired
	private Noc_applicantRepository noc_applicantRepository;
	@Autowired
	...
	public JSONObject getById(Integer id) {
		...
		try {
			dynamicValue = entityManager
					.createNativeQuery(
							"select block_name from m_block where block_id=" + entity.getSelBlockULB())
					.getSingleResult();
		} catch (Exception ex) {
			dynamicValue = "--";
		}
		...
		try {
			dynamicValue = entityManager
					.createNativeQuery(
							"select state_name from m_state where state_id=" + entity.getSelState17())
					.getSingleResult();
		} catch (Exception ex) {
			dynamicValue = "--";
		}
		...
		try {
			dynamicValue = entityManager
					.createNativeQuery(
							"select block_name from m_block where block_id=" + entity.getSelBlockULB19())
					.getSingleResult();
		} catch (Exception ex) {
			dynamicValue = "--";
		}
	...
	public JSONArray getAll(String formParams) {
		...
		for (Noc_applicant entity : noc_applicantResp) {
			...
			try {
				dynamicValue = entityManager
						.createNativeQuery(
								"select state_name from m_state where state_id=" + entity.getSelState())
						.getSingleResult();
			} catch (Exception ex) {
				dynamicValue = "--";
			}
			...
			try {
				dynamicValue = entityManager
						.createNativeQuery(
								"select gp_name from m_gp where gp_id=" + entity.getSelGPWARDNumber())
						.getSingleResult();
			} catch (Exception ex) {
				dynamicValue = "--";
			}
	...
	public static JSONArray fillselBlockULBList(EntityManager em, String jsonVal) {
		...
		String query = "Select block_id,block_name from m_block where deleted_flag = '0' and district_id="
				+ val;
		List<Object[]> dataList = em.createNativeQuery(query).getResultList();
		for (Object[] data : dataList) {
			JSONObject jsonObj = new JSONObject();
	...
	public static JSONArray fillselVillageLocalAreaNameList(EntityManager em, String jsonVal) {
		...
		String query = "Select village_id,village_name from m_village_master where deleted_flag = '0' and gp_id="
				+ val;
		List<Object[]> dataList = em.createNativeQuery(query).getResultList();
		for (Object[] data : dataList) {
			JSONObject jsonObj = new JSONObject();
