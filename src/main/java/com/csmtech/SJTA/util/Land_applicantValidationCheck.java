...
public class Land_applicantValidationCheck {
	public static String BackendValidation(JSONObject obj) {
		...
		if (errorStatus == 0 && CommonValidator.validTelCheck((String) obj.get("txtMobileNo"))) {
			errorStatus = 1;
			errMsg = "Mobile NoGiven Telephone should be  valid !";
		}
		...
		if (errorStatus == 0 && CommonValidator.isEmpty(obj.get("txtPoliceStation22").toString())) {
			errorStatus = 1;
			errMsg = "Police Station should not  be empty!";
		}
		...
		if (errorStatus == 0
				&& CommonValidator.maxLengthCheck((String) obj.get("txtHabitationStreetNoLandmark24"), 90)) {
			errorStatus = 1;
			errMsg = "Habitation/ Street No./ Landmark  maxmimum length should be 90";
		}
		if (errorStatus == 0
