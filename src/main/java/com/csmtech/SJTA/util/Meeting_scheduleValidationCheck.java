...
public class Meeting_scheduleValidationCheck {
private static final Logger logger = LoggerFactory.getLogger(Meeting_scheduleValidationCheck.class);
...
if (errorStatus == 0 && CommonValidator.BlankCheckRdoDropChk(obj.get("selDistrict48").toString())){
 errorStatus = 1;
errMsg= "District  should not be empty !";}
if (errorStatus == 0 && CommonValidator.BlankCheckRdoDropChk(obj.get("selTahasil49").toString())){
 errorStatus = 1;
errMsg= "Tahasil  should not be empty !";}
if (errorStatus == 0 && CommonValidator.BlankCheckRdoDropChk(obj.get("selVillage50").toString())){
 errorStatus = 1;
errMsg= "Village  should not be empty !";}
if (errorStatus == 0 && CommonValidator.BlankCheckRdoDropChk(obj.get("selKhatian51").toString())){
 errorStatus = 1;
errMsg= "Khatian  should not be empty !";}
if (errorStatus == 0 && CommonValidator.BlankCheckRdoDropChk(obj.get("selPlot52").toString())){
 errorStatus = 1;
errMsg= "Plot  should not be empty !";}
...
if (errorStatus == 0 &&CommonValidator.isAlphaNumericKey(obj.get("txtrMeetingPurpose54").toString())){
 errorStatus = 1;
errMsg= "Meeting Purpose should be AlphaNumeric !";}
return errMsg;
}
}