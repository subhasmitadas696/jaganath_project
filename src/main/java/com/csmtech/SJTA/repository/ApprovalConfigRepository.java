...
public class ApprovalConfigRepository {
	...
	public Integer saveApprovalConfig(JSONArray stageData, String serviceName) {
		String updateQuery = "UPDATE approval_configration SET status = '1' WHERE approval_type = '" + serviceName
				+ "'";
		entity.createNativeQuery(updateQuery).executeUpdate();

		StringBuilder queryBuilder = new StringBuilder();
		queryBuilder.append(
				"INSERT INTO approval_configration(approval_type, role_id, approval_action_ids, created_by, approval_level, timeline_days) VALUES ");

		for (int i = 0; i < stageData.length(); i++) {
			queryBuilder.append(
					"(:approvalType" + i + ", :roleId" + i + ", :actionIds" + i + ", 1, :approvalLevel" + i + ", :timelineDays" + i + ")");
