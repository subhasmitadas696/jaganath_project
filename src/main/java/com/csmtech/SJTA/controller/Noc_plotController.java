...
import com.csmtech.SJTA.util.Noc_plotValidationCheck;

@RestController
@CrossOrigin("*")
@RequestMapping("/noc")
public class Noc_plotController {
	@Autowired
	private Noc_plotService noc_plotService;
	String path = "src/storage/plot-details/";
	String data = "";

	@PostMapping("/plot-details/addEdit")
	public ResponseEntity<?> create(@RequestBody String noc_plot) throws JsonMappingException, JsonProcessingException {
		data = CommonUtil.inputStreamDecoder(noc_plot);
		JSONObject resp = new JSONObject();
		if (Noc_plotValidationCheck.BackendValidation(new JSONObject(data)) != null) {
			resp.put("status", 502);
			resp.put("errMsg", Noc_plotValidationCheck.BackendValidation(new JSONObject(data)));
		} else {
			resp = noc_plotService.save(data);
		}
	...
	public ResponseEntity<?> getById(@RequestBody String formParams) {
		...
		JSONObject entity = noc_plotService.getById(json.getInt("intId"));
	...
	public ResponseEntity<?> getAll(@RequestBody String formParams) {
		JSONArray entity = noc_plotService.getAll(CommonUtil.inputStreamDecoder(formParams));
	...
	public ResponseEntity<?> delete(@RequestBody String formParams) {
		data = CommonUtil.inputStreamDecoder(formParams);
		JSONObject json = new JSONObject(data);
		JSONObject entity = noc_plotService.deleteById(json.getInt("intId"));
